...
class GIB_DS(nn.Module):
    ...
    def __init__(self,
        ...
        self.solute_gather = GINE(self.node_input_dim, self.edge_input_dim, 
                            self.node_hidden_dim, self.num_step_message_passing,
                            )
        self.solvent_gather = GINE(self.node_input_dim, self.edge_input_dim, 
                            self.node_hidden_dim, self.num_step_message_passing,
                            )
    ...
    def forward(self, data, bottleneck = False, test = False):
        ...
        if test:
            ...
            self.solute_importance = torch.sigmoid(self.solute_importance)
            self.solvent_importance = torch.sigmoid(self.solvent_importance)
