...
class GIB_DS(nn.Module):
    def __init__(self, args, device):
        ...
        self.set2set_solute = Set2Set(6 * self.nhid, self.num_step_set2set)
        self.set2set_solvent = Set2Set(6 * self.nhid, self.num_step_set2set)
    ...
    def solvent_compress(self, solute_features):
        
        p = self.solvent_compressor(solute_features)
        temperature = 1.0
        bias = 0.0 + 0.0001  # If bias is 0, we run into problems
        eps = (bias - (1 - bias)) * torch.rand(p.size()) + (1 - bias)
        gate_inputs = torch.log(eps) - torch.log(1 - eps)
    ...
    def forward(self, data, int_map, bottleneck = False, test = False):
        ...
        if (bottleneck == True) and (test == False):
            ...
            #solute handle
            lambda_pos, p = self.solute_compress(f1_features)
            ...
            noisy_subgraphs1 = self.set2set_solute(noisy_node_feature, batch_1)
            ...
            #solvent handle
            lambda_pos, p = self.solvent_compress(f2_features)
            lambda_pos = lambda_pos.reshape(-1, 1)
            lambda_neg = 1 - lambda_pos

            static_solvent_feature = f2_features.clone().detach()
            node_feature_mean = scatter_mean(static_solvent_feature, batch_2, dim = 0)[batch_2]
            node_feature_std = scatter_std(static_solvent_feature, batch_2, dim = 0)[batch_2]

            noisy_node_feature_mean = lambda_pos * f2_features + lambda_neg * node_feature_mean
            noisy_node_feature_std = lambda_neg * node_feature_std

            noisy_node_feature = noisy_node_feature_mean + torch.rand_like(noisy_node_feature_mean) * noisy_node_feature_std
            noisy_subgraphs2 = self.set2set_solvent(noisy_node_feature, batch_2)

            epsilon = 1e-7

            KL_tensor = 0.5 * scatter_add(((noisy_node_feature_std ** 2) / (node_feature_std + epsilon) ** 2).mean(dim = 1), batch_2).reshape(-1, 1) + \
                            scatter_add((((noisy_node_feature_mean - node_feature_mean)/(node_feature_std + epsilon)) ** 2), batch_2, dim = 0)
            ...
            final_features = torch.cat((noisy_subgraphs1, noisy_subgraphs2), 1)
            scores = self.predictor(final_features)
            scores = torch.sigmoid(scores).view(-1)

            return scores, solute_KL_Loss, solvent_KL_Loss

        else:

            graph1 = self.set2set_solute(f1_features, batch_1)
            graph2 = self.set2set_solvent(f2_features, batch_2)
...
class CGIB(nn.Module):
    def __init__(self, args, device):
        ...
        self.set2set = Set2Set(6 * self.nhid, self.num_step_set2set)
