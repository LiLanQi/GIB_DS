...
from models import CGIB, CGIB_cont, GIB_DS
...
elif args.embedder == "GIB_DS":
    model = GIB_DS(args, device).to(args.device)
    ...
    with torch.autograd.detect_anomaly():
        for epoch in range(args.epochs):
            ...
            for index, batch_pair in enumerate(batches):
                ...
                #prediction, KL_Loss, pair_pred_loss = model(data, masks, bottleneck = True) #CIGIB LOSS, if needed,
                prediction, solute_KL_Loss, solvent_KL_Loss = model(data, masks, bottleneck = True)
                
                loss += F.mse_loss(prediction, data['target'], reduction='sum')
                # loss += args.beta * (KL_Loss + pair_pred_loss) #CIGIB LOSS, if needed,
                loss += args.beta * solute_KL_Loss
                loss += args.beta * solvent_KL_Loss
